...
4. Use the'check Winner'function to determine the winner for both Data1 and Data 2
...
const calcAverage = (arrayScores) => {
    let total = 0;
    let avgScore = 0;
    arrayScores.forEach((value) => {
        total += value;
    });
    return avgScore = total / arrayScores.length;
}
...
4. Bonus: Create an array'total'containing the totalvalues, so the bill + tip
...
if (objMark.bmi > objJohn.bmi) {
    ...
    console.log(`${objJohn.fullName}'s BMI (${objJohn.bmi}) and ${objMark.fullName}'s BMI (${objMark.bmi}) is the same!`);
}
*/
/*
Coding Challenge #4
